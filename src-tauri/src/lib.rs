...
async fn execute_mysql_query(connection: &DatabaseConnection, sql: &str) -> Result<QueryResult, String> {
    ...
    if is_select {
        ...
        Ok(QueryResult {
            ...
            execution_time: 0,
            message: format!("查詢成功，返回 {row_count} 行", row_count = rows.len()),
        })
    } else {
        let result = sqlx::query(sql)
            .execute(&pool)
        ...
        Ok(QueryResult {
            ...
            execution_time: 0,
            message: format!("執行成功，影響 {rows_affected} 行", rows_affected = result.rows_affected()),
        })
    }
}
    ...
    let pool = sqlx::postgres::PgPool::connect(&database_url)
        ...
        .map_err(|e| {
            ...
            if error_msg.contains("11001") || error_msg.contains("無法識別這台主機") {
                format!("DNS 解析失敗: 無法找到主機 {host}。請檢查主機名稱是否正確，或嘗試使用 IP 地址", host = connection.host)
                ...
                format!("資料庫不存在: 資料庫 '{database}' 不存在", database = connection.database)
            } else {
